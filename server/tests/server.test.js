...
describe('DELETE /todos/:id', () => {
  ...
  it('should not remove a todo which belongs to other user', (done) => {
...
describe('PATCH /todos/:id', () => {
  ...
  it('should not update the todo belongs to other user', (done) => {
    let hexId = todos[0]._id.toHexString();
    let text = `Updated text`;
    request(app)
      .patch(`/todos/${hexId}`)
    ...
    request(app)
      ...
      .end((err, res) => {
        ...
        User.findById(users[1]._id).then((doc) => {
          expect(doc.tokens[1]).to.include({
            access: 'auth',
  ...
  it('should reject invalid password', (done) => {
    request(app)
      ...
      .end((err, res) => {
        if (err) return done(err);
        User.findById(users[1]._id).then((doc) => {
          expect(doc.tokens).to.have.lengthOf(1);
          done();
        }).catch((e) => done(e));
      });
  });
